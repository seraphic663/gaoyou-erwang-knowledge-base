...
function buildSystemPrompt() {
  return [
    ...
    '这不是五段材料摘要，而是一条能被读者复述的论证。先让读者知道“本案要解释什么、反驳什么、主张什么”，再解释依据；不要让“材料状态”占据正文的中心。',
    '五步各有固定任务：发疑说清考据对象和对立说法；设问只提出要裁定的一个问题；取证逐条说出材料是什么以及它证明什么；释理说明依据怎样推出判断，不重复取证；结论先给明确判断，再补主要理由和必要的核验边界。',
    '每一步的第一句必须直接说清本步答案，不能以“本步需要说明”“可以这样理解”“在现有材料下”“当前可用的直接材料只有”等空转句开头。读者只看第一句，也应该知道这一步在说什么。每步只保留一个中心意思，短句之间不要连续堆叠多个转折。',
    ...
    '不要把“目标典籍未关联”“原典尚未确认”“检索候选不能替代证据”反复写成正文主线；最多在取证或结论末尾集中说明一次。',
    '只写材料能支持的身份和事实。材料只使用传统称谓、代称或未具名称呼时，不得依据常识补出姓名；只有材料明确具名时才可写姓名。即使姓名出现在检索候选、文件名或模型熟知的作者关系中，也不能把它当作当前案例的直接材料。',
    ...
    '为避免响应被截断并保持清楚，每项 text 控制在 520 个中文字符以内，review_questions 最多 2 条且每条不超过 100 个字符；evidence_refs 只列直接相关编号，每步最多 8 个。不要重复整段原文。',
...
function buildUserPrompt(context) {
  ...
  return [
    '请为下面的案例生成五步释证草稿。',
    '先在内部整理一条总论证：考据对象是什么、反驳谁、作者主张什么、主要依据是什么、依据如何推出结论；不要输出这条工作说明本身。',
    '严格按首页五步写，且只完成对应任务：发疑用一两句说清对象和冲突；设问只写一个要裁定的问题，不能把三个问题并在一起；取证逐条写“材料 + 它证明什么”；释理用 2 至 3 个编号短步骤写“主张—依据—推导”；结论第一句直接给判断，第二句给主要理由，必要时第三句再给核验边界。',
    '不要以“材料中……”“当前可用……”“项目尚未……”开头。先说考据对象、具体分歧或本步判断。',
    '每一步最多两段；取证涉及两条或以上材料/作用时必须使用项目符号；释理必须使用“1. …”“2. …”“3. …”。列表中的每一项都要说明作用，不要只列书名或引文。',
    '如果材料明确出现某作者对某字词或句子的解释，结论第一句必须明确归属并复述这项主张；随后再说明项目能否把它作为已独立核验的最终结论。',
    '最终结论至少回答四点：谁的判断、解释对象是什么、判断内容是什么、主要依据是什么。结论中先把这四点说完，再说核验边界；若材料没有提供其中某一点，明确指出缺哪一点，不要自行补齐或补出人物姓名。',
