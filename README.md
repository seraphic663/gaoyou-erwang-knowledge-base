...
本仓库采用“资料、方案、网站、数据引擎、参考”分层结构，便于申报、开发、数据加工与答辩并行推进。
...
- `02-研究方案`：数据库任务与字段规范、最小必要性评估
- `03-项目网站`：当前对外网站与 Railway 部署入口
- `大创`：离线数据加工区，负责原始语料解析、SQLite 实库维护与网站快照同步
...
数据加工区的当前说明见 [大创/README.md](%E5%A4%A7%E5%88%9B/README.md)。
...
- 已将旧 Flask 界面层从 `大创/` 根目录归档到 `大创/归档/flask-ui`
- 已将旧样例脚本与旧数据库归档，避免和当前主链路混用
- 当前主链路明确为：
  `大创/data2/dictionary.db` -> `npm run sync:sqlite` -> `03-项目网站/data/sqlite-snapshot.json` -> `npm start`
